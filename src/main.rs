// Here I have all the build in functions that I know abuot and I use in codding 
fn main() {
    println!("no.1 = .clone(); ::: it clones a variable");
    println!("no.2 = .push(); ::: it pushes Strings of data into a vector");
    println!("no.3 = .pop(); ::: it cuts a block of data from variables/vectors");
    println!("no.4 = .new(); ::: it creats a new code");
    println!("no.5 = .insert(); ::: it inserts data into a hashmap");
    println!("no.6 = .collect(); ::: it collets and compiles two inbuild fuctons in to a hashmap");
    println!("no.7 = .get(); ::: it brings variable no. 2 into variable no.1");
    println!("no.8 = .iter(); ::: it is used with the name of the vector/varbale being combined");
    println!("no.9 = .zip(); ::: it it combines a vector/variable with variable/vector");
    println!("no.10 = .entry(); ::: it pushes data in hasmap");
    println!("no.11 = .or_insert(); ::: it inserts data into hashmap variable");
    println!("no.12 = .split_whitespace(); ::: it splits a words in a string (partition done by spaces);
    println!("no.13 = .to_string(); ::: it converts str into strings);
    println!("no.14 = .push_str(); ::: it pushes strings");
    println!("no.14 = .len(); ::: it counts the leters in a string");
    println!("no.14 = .bytes(); ::: it tells the bytes taken by each letter in a string");
    println!("no.14 = .chars(); ::: it breaks apart every letter in a string");


}
